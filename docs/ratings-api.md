...
- `open_hours` 의 배열은 **수집한 날부터 앞으로 7일**이다. 여기에 함정이 둘 있다.
  - 0 번이 "오늘" 이 아니라 "수집한 날" 이다. 수집은 주 1회 1/4 씩 돌아 한 장소의 배열이
    최대 4주 고정되므로, 그대로 읽으면 금요일에 화요일 영업시간을 적용하게 된다.
    그래서 `ratings.json` 에 수집 요일(`hoursDay`)을 같이 싣고 `src/lib/hours.ts` 가
    `(보는 요일 − 수집 요일 + 7) % 7` 로 칸을 민다.
  - 어제가 없다. 자정을 넘겨 여는 가게를 새벽에 판정할 때는 오늘의 마감 시각을 그대로 쓴다.
